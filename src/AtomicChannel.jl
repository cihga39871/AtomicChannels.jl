...
A cell in the `AtomicChannel`. The `state` can be empty (0), filled (1), busy (2; being written/cleared), or not applicable (3; for single-threaded mode). The `value` field holds the item when the cell is filled or `nothing` when it is empty.
"""
mutable struct AtomicCell{T}
...
const CELL_NA     = 0x03  # for single threaded mode

"""
    AtomicChannel{T<:Any, B=Threads.nthreads() > 1}(capacity::Int)
    AtomicChannel(capacity::Int) -> AtomicChannel{Any, Threads.nthreads() > 1}
...
- `B` is an auto-filled boolean value parameter that controls whether to use atomic operations. It defaults to `true` if `Threads.nthreads() > 1`, and `false` for single-threaded contexts.
...
```
"""
struct AtomicChannel{T, B} <: AbstractChannel{T}
    ...
    function AtomicChannel{T, B}(head::Threads.Atomic{Int}, tail::Threads.Atomic{Int}, n_filled::Threads.Atomic{Int}, n_free::Threads.Atomic{Int}, capacity::Int, cells::Vector{AtomicCell{T}}) where {T, B}
        @assert B isa Bool "AtomicChannel: B must be a Bool value parameter"
...
function AtomicChannel{T}(capacity::Int) where T
    B = Threads.nthreads() > 1
    AtomicChannel{T, B}(capacity)
end
function AtomicChannel(capacity::Int) 
    B = Threads.nthreads() > 1
    AtomicChannel{Any, B}(capacity)
end

"""
    convert(::Type{AtomicChannel{T, A}}, chnl::AtomicChannel{T, B}) where {T, A, B}

Convert an `AtomicChannel` with boolean parameter `B` to another `AtomicChannel` with the same item type `T` but a different boolean parameter `A`. This is useful for converting between single-thread optimized and multi-thread optimized versions of the channel. All items currently in the channel will be moved to the new channel during conversion.
"""
function Base.convert(::Type{AtomicChannel{T, A}}, chnl::AtomicChannel{T, B}) where {T, A, B}
    new_chnl = AtomicChannel{T, A}(chnl.capacity)
    for item in chnl
        put!(new_chnl, convert(T, item))
    end
    return new_chnl
end
...
@inline function _acquire_ring_index!(chnl::AtomicChannel{T, true}, idx::Threads.Atomic{Int}) where T<:Any
    capacity = chnl.capacity
...
"""
function Base.put!(chnl::AtomicChannel{T, true}, item::T) where T<:Any
    _acquire_token!(chnl, chnl.n_free)
...
function Base.put!(chnl::AtomicChannel{T, false}, item::T) where T<:Any
    _acquire_token!(chnl, chnl.n_free)
    slot = _acquire_ring_index!(chnl, chnl.tail)
    cell = @inbounds chnl.cells[slot]
...
"""
function tryput!(chnl::AtomicChannel{T, true}, item::T) where T<:Any
    _try_acquire_token!(chnl, chnl.n_free) || return false
...
function tryput!(chnl::AtomicChannel{T, false}, item::T) where T<:Any
    _try_acquire_token!(chnl, chnl.n_free) || return false
...
function tryput!(reset_func::Base.Callable, chnl::AtomicChannel{T, true}, item::T) where T<:Any
    _try_acquire_token!(chnl, chnl.n_free) || return false
...
function tryput!(reset_func::Base.Callable, chnl::AtomicChannel{T, false}, item::T) where T<:Any
    _try_acquire_token!(chnl, chnl.n_free) || return false
...
See also: [`tryput!`](@ref) and [`trytake!`](@ref) for non-blocking versions of put and take.
"""
function Base.take!(chnl::AtomicChannel{T, true}) where T<:Any
...
function Base.take!(chnl::AtomicChannel{T, false}) where T<:Any
    _acquire_token!(chnl, chnl.n_filled)
    slot = _acquire_ring_index!(chnl, chnl.head)
    cell = @inbounds chnl.cells[slot]
...
Edge case:
 - If you put a `nothing` into the chnl, it will be treated as a valid item and returned.
"""
function trytake!(chnl::AtomicChannel{T, true}) where T<:Any
    _try_acquire_token!(chnl, chnl.n_filled) || return nothing
...
@inline Base.length(chnl::AtomicChannel{<:Any, <:Any}) = chnl.capacity

# API compatibility with Base.Channel
@inline Base.isopen(chnl::AtomicChannel{<:Any, <:Any}) = true
@inline Base.close(chnl::AtomicChannel{<:Any, <:Any}) = nothing
@inline Base.close(chnl::AtomicChannel{<:Any, <:Any}, excp::Exception) = nothing
...
@inline Base.lock(f::Function, chnl::AtomicChannel{<:Any, <:Any}) = f()
@inline Base.unlock(chnl::AtomicChannel{<:Any, <:Any}) = nothing

Base.eltype(::Type{<:AtomicChannel{T, <:Any}}) where T = T

function Base.wait(chnl::AtomicChannel{T, <:Any}) where T
    _acquire_token!(chnl, chnl.n_filled)
    Threads.atomic_add!(chnl.n_filled, 1)
    return nothing
end
...
function Base.iterate(chnl::AtomicChannel{T, true}, state=nothing) where T
    _try_acquire_token!(chnl, chnl.n_filled) || return nothing
...
function Base.iterate(chnl::AtomicChannel{T, false}, state=nothing) where T<:Any
    _try_acquire_token!(chnl, chnl.n_filled) || return nothing
...
function Base.show(io::IO, cell::AtomicCell{T}) where T
    ...
    elseif state == CELL_BUSY
        print(io, "AtomicCell{", T, "}(busy)")
    else
        print(io, "AtomicCell{", T, "}(value=", value, ")")
    end
end
