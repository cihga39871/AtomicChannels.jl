...
@testset "AtomicChannel" begin
    ...
    @testset "AtomicChannel concurrent producers consumers" begin
        chnl = AtomicChannel{Int, true}(64)
        n_items = 2000
    ...
    @testset "AtomicChannel pretty print" begin
        for B in (false, true)
            ...
            B_str = B ? "true" : "false"
            @test occursin("AtomicChannel{Int64, $B_str}(2/3)", s_short)
            @test occursin("AtomicChannel{Int64, $B_str} with 2/3 items", s_plain)
            B && @test occursin("slots", s_plain)
        end
        ...
        cell.state[] = 3
        @test occursin("AtomicCell{Int64}(value=", sprint(show, cell))

        chnl_large = AtomicChannel{Int, true}(9999)
    ...
    @testset "AtomicChannel ring-index wrap-around" begin
        for B in (false, true)
            ...
            if B
                ...
                @test chnl.cells[1].value[] == 12
                @test chnl.cells[2].value[] == 13
                @test chnl.cells[3].value[] == 14
                @test chnl.cells[4].value[] === nothing
                @test chnl.cells[5].value[] === nothing
                @test chnl.cells[6].value[] == 11
            end
    ...
    @testset "AtomicChannel ring-index large value reset" begin
        ...
        chnl = AtomicChannel{Int,true}(capacity)
        slot = AtomicChannels._acquire_ring_index!(chnl, idx)
    ...
    @testset "AtomicChannel compatibility and utility API" begin
        for B in (false, true)
            chnl = AtomicChannel{Int,B}(2)

            @test length(chnl) == 2
            @test eltype(AtomicChannel{Int,B}) == Int
    ...
    @testset "AtomicChannel tryput! with reset function" begin
        for B in (false, true)
            chnl = AtomicChannel{Vector{Int},B}(1)
